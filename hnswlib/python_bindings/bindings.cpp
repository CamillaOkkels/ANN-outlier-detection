...
class Index {
 public:
    ...
    std::vector<float> detectOutliers(bool contrast=false) {
        auto res = appr_alg->detectOutliers(contrast);
        return res;
    }

    std::vector<std::vector<unsigned int>> getAdj() {
        return appr_alg->getAdj();
    }
...
PYBIND11_PLUGIN(hnswlib) {
        ...
        .def("detect_outliers", &Index<float>::detectOutliers, py::arg("contrast"))
        .def("get_adj", &Index<float>::getAdj)
        .def("knn_query",
